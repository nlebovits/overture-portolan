...
A Portolan mirror of Overture Maps. It holds metadata only. Every `data` and
`visual` asset points at Overture's public buckets, and this repository copies
no bytes.

### The publish boundary

`catalog/` is the published catalog. Everything in it is published, and nothing
outside it ever is. `.github/workflows/pages.yml` uploads that directory and
nothing else. Do not move a file into `catalog/` to make it publish.

### Publication and its two accepted deviations

A push to `main` deploys `catalog/` to GitHub Pages. This catalog uses neither
`portolan push` nor the template's `tools/publish.py`, and it uploads nothing to
an object store.

That choice costs two conformance findings, PTL-LIV-004 and PTL-LIV-005, both
recorded in `docs/conformance.md` and tracked by portolan-spec#183. Read
`docs/publication.md` before any change to hosting, the item shape, or the
asset formats. The three decisions are coupled, and the reasoning is not
recoverable from the code.

The rule that follows from them: **never put a cloud-native asset on this
host.** GeoParquet, COG, PMTiles, and COPC all need range reads that a browser
cannot perform here. That is why the catalog ships plain item JSON rather than a
STAC-GeoParquet item mirror, despite PORTO-FMT-038.

### Data never enters git

Never commit a GeoParquet, COG, PMTiles, Zarr, or COPC file. Git keeps every
version of a binary forever, and deletion later reclaims nothing. This catalog
has no reason to hold one: the data belongs to Overture.

### Generated metadata

`catalog/` is generated by the scripts in `tools/`. Edit the generator or the
authored inputs, never the generated JSON. `portolan add` overwrites authored
fields such as `title`, `license`, and `providers`, so authored metadata is
re-applied as a last pass.

### The conformance allow-list

`ACCEPTED` in `tests/test_conformance.py` holds exactly the two ids above. Never
...
measured from the data. Column meanings come from Overture's published JSON
Schema at a pinned tag, and the tag is recorded. An invented join key or column
name produces a confident wrong answer that nothing downstream catches.
