...
Overture Maps `buildings` theme, `building_part` type, release `2026-08-19.0`.
1 GeoParquet parts holding 4,339,150 rows. Geometry is WKB in EPSG:4326.
...
FROM read_parquet('s3://overturemaps-us-west-2/release/2026-08-19.0/theme=buildings/type=building_part/*.parquet');
```
...
geometry in 4,339,150 rows.

## Row groups

Parts hold 256 to
256 row groups. The largest holds
24,949 rows, which is inside
...
theme. This collection's features are in the `building_part` layer. Declare
