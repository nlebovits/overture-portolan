...
Overture Maps `divisions` theme, `division_boundary` type, release `2026-08-19.0`.
1 GeoParquet parts holding 87,533 rows. Geometry is WKB in EPSG:4326.
...
FROM read_parquet('s3://overturemaps-us-west-2/release/2026-08-19.0/theme=divisions/type=division_boundary/*.parquet');
```
...
```sql
SELECT count(*) AS rows
FROM read_parquet('https://overturemaps-us-west-2.s3.us-west-2.amazonaws.com/release/2026-08-19.0/theme=divisions/type=division_boundary/part-00000-57556b35-50bb-5108-bd25-fca8c4ef9d56-c000.zstd.parquet');
```
...
geometry in 87,533 rows.

## Row groups

Parts hold 64 to
64 row groups. The largest holds
1,584 rows, which is inside
...
theme. This collection's features are in the `division_boundary` layer. Declare
