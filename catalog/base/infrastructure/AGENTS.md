...
Overture Maps `base` theme, `infrastructure` type, release `2026-08-19.0`.
16 GeoParquet parts holding 155,513,336 rows. Geometry is WKB in EPSG:4326.
...
FROM read_parquet('s3://overturemaps-us-west-2/release/2026-08-19.0/theme=base/type=infrastructure/*.parquet');
```
...
```sql
SELECT count(*) AS rows
FROM read_parquet('https://overturemaps-us-west-2.s3.us-west-2.amazonaws.com/release/2026-08-19.0/theme=base/type=infrastructure/part-00000-8f1aee10-0ea7-5bf2-873a-0b99d39adba3-c000.zstd.parquet');
```
...
geometry in 155,513,336 rows.

## Row Groups

Parts hold 256 to
256 row groups. The largest holds
67,105 rows, which is inside
...
- **`subtype`**: `aerialway`, `airport`, `barrier`, `bridge`, `communication`, `emergency`, `manhole`, `pedestrian`, `pier`, `power`, `quay`, `recreation`, and 6 more
...
theme. This collection's features are in the `infrastructure` layer. Declare
